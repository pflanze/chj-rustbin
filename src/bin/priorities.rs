...
use chrono::{
    DateTime, Datelike, Duration, Local, NaiveDateTime, NaiveTime, Timelike,
    Utc, Weekday,
};
use clap::Parser;
use kstring::KString;
...
use chj_rustbin::{
    ...
    io::file_path_type::{
        recursive_file_path_types_iter, FilePathType, ItemOptions,
    },
    parse::{
        parse_error::{
            Backing, IntoOwningBacking, ParseContext, ParseError,
            StringParseContext,
        },
...
impl Default for Importance {
    fn default() -> Self {
        Self {
            level: DEFAULT_IMPORTANCE_LEVEL,
        }
    }
}
...
impl<'s, C: ParseContext> Priority<C> {
    ...
    pub fn custom_eq(
        ...
        other: &Self,
        cmp_ndtwowy: impl Fn(
            &NaiveDateTimeWithOrWithoutYear<C>,
            &NaiveDateTimeWithOrWithoutYear<C>,
        ) -> bool,
    ) -> bool {
        match self {
...
fn add_year_from_mtime<C: ParseContext + Clone>(
    ...
    let seconds_since_mtime =
        |ndt: NaiveDateTime| ndt.signed_duration_since(mtime_ndt).num_seconds();
...
fn t_add_year_from_mtime() {
    ...
    assert_eq!(t(&noyear(1, 30), ndt(2024, 2, 1), &range1), ok(2024, 1, 30));
    assert_eq!(t(&noyear(3, 30), ndt(2024, 2, 1), &range1), ok(2024, 3, 30));
    assert_eq!(
        t(&noyear(1, 30), ndt(2023, 12, 1), &range1),
        ok(2024, 1, 30)
    );
    assert_eq!(
        t(&noyear(2, 28), ndt(2023, 12, 20), &range1),
        ok(2024, 2, 28)
    );
    assert_eq!(
        t(&noyear(2, 28), ndt(2023, 12, 1), &range1),
        ok(2024, 2, 28)
    );
    ...
    assert_eq!(
        t(&noyear(12, 28), ndt(2024, 1, 15), &range1),
        ok(2023, 12, 28)
    );
    assert_eq!(
        t(&noyear(12, 16), ndt(2024, 1, 15), &range1),
        ok(2023, 12, 16)
    );
    ...
    assert_eq!(
        t(&noyear(2, 30), ndt(2024, 2, 15), &range1)
            .unwrap_err()
            .message,
...
fn add_year_from_basetime<C: ParseContext + Clone>(
    ...
    match base_noyear.compare(&noyear) {
        ...
        Ordering::Greater => noyear
            .clone()
            .with_year(base.year().checked_add(1).expect("OS-limited")),
    }
}
...
fn t_add_year_from_basetime() {
    ...
    assert_eq!(
        t(&noyear(2, 29), ndt(2024, 3, 1)).unwrap_err().message,
...
pub fn priority_level<C: ParseContext + Clone>(
    ...
    let priority_for_time_left = |time_to_target: Duration| {
        ...
        let time_weeks: f32 =
            (time_secs as f32) / programming_secs_per_week * 5.; // XXXXHACK
    ...
    match priority {
        Priority::Date(ndtwowy) => {
            let target_date = match ndtwowy {
                NaiveDateTimeWithOrWithoutYear::NaiveDateTime(ndt) => *ndt,
                NaiveDateTimeWithOrWithoutYear::NaiveDateTimeWithoutYear(
                    ndt_noyear,
                ) => {
                    let from_mtime = || {
                        add_year_from_mtime(
                    ...
                    if let Some(key) = key {
                        match key {
                            ...
                            DependencyKey::NaiveDateTime(ndt_file) => T!(
                                add_year_from_basetime(ndt_noyear, *ndt_file)
                            )?,
                        }
                    } else {
                        T!(from_mtime())?
            ...
            let time_to_target: Duration =
                target_date.signed_duration_since(now);
        ...
        Priority::Today => {
            let target_time = now
                ...
                .with_second(0)
                .unwrap();
            Ok(priority_for_time_left(
                target_time.signed_duration_since(now),
            ))
        }
        Priority::Tonight => {
            let target_time = now
...
impl<'s, B: Backing + Debug> FromParseableStr<'s, B>
    for Priority<StringParseContext<&'s B>>
where
    &'s B: Backing,
{
    fn from_parseable_str(
        ...
        if let Ok((ndt_wowy, rest)) = parse_dat(
            &s.trim(),
            &flexible_parse_dat_options(Some(
...
impl Dependencies {
        ...
        &'s B: Backing,
    {
        ...
        for s in s
            ...
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
        // optim: only the *last* segment can be empty
        {
            keys.insert(
                (
...
impl WorkflowStatus {
    ...
    const INSTANCES: &'static [WorkflowStatus] = {
        use WorkflowStatus::*;
        &[
            None, Open, Done, Obsolete, Wontfix, Dupe, Future, Archived,
            Notforme, Applied, Active, Rejected,
        ]
    };
    ...
    pub fn is_active(self) -> bool {
        match self {
            ...
            WorkflowStatus::Archived => {
                unreachable!("never stored, instead translated to bool")
            }
        ...
        for dir in anc {
            if let Some(file_name) = dir.file_name() {
                match file_name.to_str() {
                    ...
                    None => warning!(
                        "cannot decode file name {file_name:?} \
...
impl<C: ParseContext + Clone> TaskInfo<C> {
    ...
    fn priority_level_at(
        &self,
        now: NaiveDateTime,
    ) -> Result<f32, ParseError<C>> {
        priority_level(
            &self.declarations.priority,
    ...
    fn set_initial_priority_level_for(
        &self,
        now: NaiveDateTime,
    ) -> Result<(), ParseError<C>> {
        assert!(self.calculated_priority.get().is_none());
        self.calculated_priority
            .set(Some(T!(self.priority_level_at(now))?));
        Ok(())
    }
...
impl<C: ParseContext + Clone> PartialOrd for TaskInfo<C> {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        self.calculated_priority
            .get()
            .unwrap()
            .partial_cmp(&other.calculated_priority.get().unwrap())
    }
}
...
impl<'s, B: Backing + Debug>
    TaskInfoDeclarationsBuilder<StringParseContext<&'s B>>
where
    &'s B: Backing + Debug,
{
    fn old_err(
...
{
    s.drop_whitespace()
        .expect_str_or_eos(needle)
        .map(|v| v.drop_whitespace())
        .map_err(compose(ParseError::from, |e| {
            e.message_append(" or the end of the input segment")
        }))
}
...
fn take_one_value_from<'s, B: Backing>(
    ...
    (ParseableStr<'s, B>, ParseableStr<'s, B>),
...
fn t_take_one_value_from() {
    ...
    let t = |s| {
        take_one_value_from(ParseableStr {
            backing: &backing,
            position: 0,
            s,
        })
    ...
    &'s B: Backing,
{
    if let Some(rest) = rest.drop_str(":") {
        ...
        match ident.s {
            ...
            "p" | "prio" | "priority" | "due" => {
                parse_to!(set_priority, Priority)
            }
            "d" | "dep" | "depends" | "dependencies" => {
                parse_to!(set_dependencies, Dependencies)
            }
...
fn parse_inside<'s, B: Backing + Debug>(
    ...
    TaskInfoDeclarations<StringParseContext<&'s B>>,
    ParseError<StringParseContext<&'s B>>,
>
where
    &'s B: Backing,
        ...
        match p.take_identifier() {
            Ok((ident, rest)) => {
                if rest.starts_with(":") {
                    ...
                    if let Some((task_size, priority)) =
                        inside_parse_class(&ident)
                    {
...
fn parse_time_wday<'s, B: Backing>(
    ...
    ((u8, u8, u8), Option<(Weekday, usize)>, ParseableStr<'s, B>),
    (
        ParseTimeWdayErrorKind,
        ParseError<StringParseContext<&'s B>>,
    ),
>
where
    &'s B: Backing,
{
    ...
    let weekday_result =
        (|| -> Result<((Weekday, usize), ParseableStr<_>), ParseError<_>> {
            // "_Sun"
            let rest =
                T!(rest.expect_separator(&options.separator_between_parts))?;
            let (wdaystr, rest) =
    ...
    match weekday_result {
        Ok((weekday_and_position, rest)) => {
            Ok((time, Some(weekday_and_position), rest))
        }
        Err(e) => {
            if options.weekday_is_optional {
                Ok((time, None, rest))
            } else {
    ...
    &'s B: Backing,
{
    ...
    let (month, rest) = T!(rest.take_nrange_while(
        ...
        2,
        is_ascii_digit_char,
        "digit as part of month number"
    ))?;
    let month = u8::from_str(month.s).map_err(|e| {
        parse_error! {
    ...
    let (day, rest) = T!(rest.take_nrange_while(
        ...
        2,
        is_ascii_digit_char,
        "digit as part of day number"
    ))?;
    let day = u8::from_str(day.s).map_err(|e| {
        parse_error! {
...
fn parse_dat<'s, B: Backing + Debug>(
    ...
    (
        NaiveDateTimeWithOrWithoutYear<StringParseContext<&'s B>>,
        ParseableStr<'s, B>,
    ),
    ParseError<StringParseContext<&'s B>>,
>
    ...
    &'s B: Backing,
{
    match T!(s.take_n_while(
        ...
        Ok((year, rest)) => {
            ...
            Ok((
                NaiveDateTimeWithOrWithoutYear::NaiveDateTime(datetime),
                rest,
            ))
        }
        Err(_e1) => {
    ...
    &'s B: Backing,
{
    let (ndt, rest) = parse_dat(
        s,
        &flexible_parse_dat_options(if time_is_optional {
...
const fn parse_path_parse_dat_options(
    weekday_is_optional: bool,
) -> ParseDatOptions {
    ParseDatOptions {
        default_time: None,
...
const PARSE_DAT_OPTIONS_FOR_PATH: ParseDatOptions =
    parse_path_parse_dat_options(true);
const PARSE_DAT_OPTIONS_FOR_INSIDE: ParseDatOptions =
    parse_path_parse_dat_options(false);
...
fn t_find_all_markers() {
    let backing = String::new();
    let t = |s: &'static str| {
        find_all_markers(&ParseableStr {
            backing: &backing,
            position: 0,
            s,
        })
        .into_iter()
        .map(|(status, string, _rest)| (status, string.position))
        .collect::<Vec<_>>()
    };
    use WorkflowStatus::*;
    assert_eq!(t("foo bar"), []);
    ...
    assert_eq!(
        t("foo TODO_ DONE POPEN DONE4 /DONE+"),
        [(Done, 10), (Done, 28)]
    );
}
...
fn find_marker_status<'s, B: Backing + Debug>(
    ...
    (MarkerStatus<StringParseContext<&'s B>>, bool),
    ParseError<StringParseContext<&'s B>>,
>
where
    &'s B: Backing,
{
        ...
        match status {
            ...
            WorkflowStatus::Open => {
                if previous_was_open {
                    ...
                    open =
                        Some((decl.unwrap_or_else(Default::default), string));
    ...
    if let Some(info) = open {
        Ok((
            MarkerStatus::Open((
                info.0,
                last_status.expect("set when `open` was set"),
            )),
            is_archived,
        ))
    } else {
...
fn t_find_marker_status() {
    ...
    assert_eq!(
        v.task_info_declarations(),
        Some(&TaskInfoDeclarations {
    ...
    let (v, is_archived) =
        t("foo ARCHIVED OPEN{1, importance: 3} FUTURE{2} DONE").unwrap();
    assert!(is_archived);
    assert_eq!(v.workflow_status(), Some(WorkflowStatus::Done));
    assert_eq!(
        v.task_info_declarations(),
    ...
    let (v, is_archived) =
        t("foo OPEN{1, importance: 3} DONE FUTURE{2}").unwrap();
    assert!(!is_archived);
    assert_eq!(v.workflow_status(), Some(WorkflowStatus::Future));
    assert_eq!(
        v.task_info_declarations(),
...
fn parse_path(
    id: usize,
    verbose: bool,
    region: &Region<PathBuf>,
    item: &FilePathType,
    show_backtrace: bool,
) -> Result<TaskInfo<StringParseContext<String>>> {
    let path = item.to_path_buf(region);
    if verbose {
        eprintln!("parse_path({path:?})")
    };
    let backing: String = item
        .file_name
        .to_str()
        .ok_or_else(|| {
            anyhow!(
                "the file name in path {path:?} does not have valid encoding"
            )
        })?
        .into();
            ...
            for dependency in ti.declarations.dependencies.iter() {
                if let Some(dependency_ti) = taskinfo_by_key.get(dependency) {
                    if !list_of_seen.contains(&dependency_ti.id) {
                        dependency_ti.set_as_dependency_for_priority(
                            ti.calculated_priority(),
                        );
                    }
                } else {
                    ...
                    warning!(
                        "unknown dependency {dependency:?} in: {:?}",
                        ti.path
                    );
                    errors += 1;
                }
...
mod tests {
    ...
    fn t_parse_priority() {
        ...
        fn t_<'t>(
            ...
            Priority::from_parseable_str(&ParseableStr {
                backing,
                position: 0,
                s,
            })
        ...
        assert_eq!(
            te("2024-11-01 12:00:01 a"),
            "garbage after date/time at \"a\""
        );
        ...
        assert_eq!(
            te("2024-11-01 2:0:01  "),
            "garbage after date/time at \"2:0:01  \""
        );
        assert_eq!(
            te("2024-11-01 0200:01  "),
            "garbage after date/time at \":01  \""
        );
        assert_eq!(t("2024-11-01 0200  "), ymd_hms(2024, 11, 01, 2, 0, 0));
        assert_eq!(
            te("2024-11-01 200  "),
            "garbage after date/time at \"200  \""
        );
    ...
    fn t_parse_date_time_argument() {
        ...
        let ok: Result<
            NaiveDateTimeWithOrWithoutYear<StringParseContext<String>>,
            _,
        > = Ok(NaiveDateTimeWithOrWithoutYear::NaiveDateTime(
            NaiveDateTime::new(
        ...
        fn _t<'t>(
            ...
            NaiveDateTimeWithOrWithoutYear<StringParseContext<&'t String>>,
            ParseError<StringParseContext<&'t String>>,
        > {
            parse_date_time_argument(
                &ParseableStr {
                    backing,
                    position: 0,
                    s,
