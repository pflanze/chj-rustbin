...
use std::{collections::HashSet, ffi::{OsString, OsStr}, os::unix::prelude::OsStrExt};
...
impl Excludes {
    pub fn filename_is_excluded(&self, file_name: &OsStr, is_dir: bool) -> bool {
        (self.exclude_dot_files && filename_is_dot(&file_name))
            || (self.exclude_emacs_backups && filename_is_emacs_backup(&file_name))
            || (if is_dir { &self.dirs } else { &self.files }).contains(file_name)
    }
}
