                ...
                let target =
                    match remove_base {
                        Some(ref remove_base) => {
                            ...
                            if let Some(pos) = targetos.starts_with_to_pos(
                                remove_base) {
...
fn main() {
    let app= App::new ("symlinks-index")
        ...
        .about("On startup creates an in-memory index of the symlinks contained in $dirpath, then reads target-paths from stdin and writes the source-paths that link to the target-path to stdout. The end of an answer group is indicated by an empty string written to stdout.  By default, entries are terminated by newline characters.")
        ...
        .arg (Arg::with_name ("null as read+write terminator")
              .short ("0")
              .help ("use the null byte as record terminator for reading and writing"))
        .arg (Arg::with_name ("remove base")
              .long ("remove-base")
              .help ("remove base from the target paths before putting them into the index (simply substring it (for now))")
              .value_name("PATH"))
    ...
    let remove_base =
        match args.value_of("remove base") {
    ...
    if let Some(mut dirpaths) = args.values_of("directory-paths") {
        match dirs_index(debug, &mut dirpaths, remove_base) {
            Ok(items_from_target) => {
                if debug { eprintln!("items_from_target = {:?}",
                                     &items_from_target) };
                match serve(debug, stdin(), stdout(), items_from_target,
